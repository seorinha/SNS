		...
		<div class="write-box border rounded m-3">
			...
			<div class="d-flex justify-content-between">
				<div class="file-upload d-flex">
					<%--file태그를 숨겨두고 이미지를 클릭하면 file태그를 클릭한 효과 --%>
					<input type="file" id="file" accept=".jpg, .png, .gif, .jpeg">
					
					<a href="#" id="fileUploadBtn"><img width="35" src="https://cdn4.iconfinder.com/data/icons/ionicons/512/icon-image-512.png"></a>
					
					<%--업로드 된 임시 파일명 노출 --%>
					<div id="fileName" class="ml-2">sun.png</div>
				</div>
...
$(document).ready(function() {
	//파일 이미지를 클릭 할 때 숨겨져있던 type="file"을 동작시킨다
	$('#fileUploadBtn').on('click', function(e) {
		e.preventDefault();  //a태그의 화면 상단으로 올라가는 현상 방지
		$('#file').click();
	});
	...
	$('#file').on('change', function(e) {
		...
		if (ext != 'jpg' && ext != 'gif' && ext != 'png' && ext != 'jpeg') {
			...
			$('#file').val(""); //파일 태그에 파일 제거 (보이지 않지만 업로드 될수 있으므로 주의해야한다)
	...
	$('#writeBtn').on('click', funcion() {
		...
		$.ajax({
			...
			, error: function(e) {
				alert("글 저장에 실패했습니다. 관리자에게 문의하세요");
			}
		}); //---ajax 끝
	});
